...
import { MAX_MESSAGES_IN_BUFFER, MESSAGE_LIMIT_PER_ROOM } from "../config/constants.js";
...
export const addMessage = async (userId: ObjectId, roomId: string, message: string) => {
    if(messagesToSave.length > MAX_MESSAGES_IN_BUFFER || messagesToDelete.length > MAX_MESSAGES_IN_BUFFER) {
